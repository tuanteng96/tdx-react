...
function PickerPoint({ children, rowData, data }) {
                                ...
                                <div className='w-[120px] flex flex-col items-end justify-between'>
                                  {item.Value < 0 && (
                                    <div
                                      className={clsx(
                                        'text-sm',
                                        item.Status !== 'HOAN_THANH' ? 'text-warning' : 'text-success'
                                      )}
                                    >
                                      {item.Status !== 'HOAN_THANH' ? 'Đang xử lý' : 'Hoàn thành'}
                                    </div>
                                  )}
                                  <div className='font-semibold'>
                                    {item.Value > 0 ? '+' : '-'}
                                    {formatString.formatPoint(Math.abs(item.Value))}
                                  </div>
                                </div>
                              </div>
                            ))}
